...
dataset = pd.read_csv('./data/training.csv', names=['thread_quota', 'packet_size', 'bandwidth_tx'])
...
input_df = pd.read_csv('./data/input.csv', names=['packet_size', 'bandwidth_tx'], delimiter=',')
...
with open('./data/output.csv','w', newline="") as f:
    ...
    for i in range(len(output_data)):
        # 'quota', 'packet_size', 'bandwidth_tx'
        makewrite.writerow([output_data[i][0], input_arr[i][0], input_arr[i][1]])
...
joblib.dump(clf_random_cv, "./model/test")
clf = joblib.load("./model/test")
